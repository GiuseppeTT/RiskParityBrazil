...
analyze_data <- function(
    ...
    gain <- list(
        ...
        dca_multiple_plot = plot_dca_multiple(data),
...
plot_price <- function(
    ...
    plot <-
        ...
        ggplot2::scale_y_log10(n.breaks = 10) +
        base_theme()
...
plot_dca_multiple <- function(
    ...
    data <-
        data %>%
        tidyr::complete(tidyr::nesting(type, asset), .data$date, fill = list(return = 0))

    data <-
        data %>%
        convert_return_to_dca_multiple()

    plot <-
        data %>%
        ggplot2::ggplot(ggplot2::aes(x = .data$date, y = .data$DCA_multiple, color = .data$asset)) +
        ggplot2::geom_line(ggplot2::aes(size = .data$type, linetype = .data$type)) +
        ggplot2::scale_y_log10(n.breaks = 10) +
        base_theme()
